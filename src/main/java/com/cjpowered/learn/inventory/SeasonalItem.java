...
public class SeasonalItem implements Item {
	...
	public Order createOrder(final LocalDate when, InventoryDatabase database, MarketingInfo marketingInfo) {
		...
		if (orderAllowed){
			if(inSeason && onSale){
				double packagesToOrderInSeason = Math.ceil((double)((wantOnHand * 2) - onHand - onOrder)/(double) unitsPerPackage);
				double packagesToOrderOnSale = Math.ceil((double)(wantOnHand + 20 - onHand - onOrder)/(double) unitsPerPackage);
				...
				double packagesToOrder = Math.ceil((double)((wantOnHand * 2) - onHand - onOrder)/(double) unitsPerPackage);
				toOrder = ((wantOnHand * 2) - onHand - onOrder) > 0 ? (int) packagesToOrder * unitsPerPackage : 0;
			} else {
